"""
ensemble_preprocess.py

_MyEXT_tagger_ensemble_worker (tew_backends/preprocess.py) からこのworker向けに移植した
前処理仕様レジストリ・タグ/カテゴリパーサー。CUDA/torch固有コードを含まない純粋な
numpy/PIL/JSON処理のため、OpenVINO(iGPU)venv側でもそのまま動作する。

対象は tagger_ensemble_worker の8モデルのうち、ONNXバックエンドの6モデルのみ
(cl_v1, cl_v2, dtq_l16, dtq_b16, oppai_v11, wd_eva02_l)。at_eva02/at_convnext_huge は
torch/timmバックエンドでONNX化されていないため対象外(ensemble_catalog.py 参照)。

モデルごとの前処理値(mean/std/パディング色/BGR有無等)の出典・注記は元ファイル
(_MyEXT_tagger_ensemble_worker/tew_backends/preprocess.py)を参照。
"""
...
logger = logging.getLogger("ComfyUI_Tagger_Worker")
...
# モデルごとの前処理仕様。対象はONNXバックエンドの6モデルのみ(上記ヘッダ参照)。
MODEL_REGISTRY: Dict[str, PreprocessSpec] = {
    ...
    "cl_v1": PreprocessSpec(  # 【実機エラーから特定】input_size=448(384ではない)
